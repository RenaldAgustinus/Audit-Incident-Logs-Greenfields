...
@if(session('success'))
    <div class="bg-green-100 border-l-4 border-green-500 text-green-700 p-4 rounded shadow-sm mb-6 flex justify-between items-center">
        ...
        <button onclick="this.parentElement.style.display='none'" class="text-green-700 hover:text-green-900 outline-none">
...
@if($errors->any())
    <div class="bg-red-100 border-l-4 border-red-500 text-red-700 p-4 rounded shadow-sm mb-6 flex justify-between items-start">
        ...
        <button onclick="this.parentElement.style.display='none'" class="text-red-700 hover:text-red-900 mt-0.5 outline-none">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path></svg>
        </button>
    </div>
@endif
...
<div class="bg-white p-4 rounded-lg shadow-sm border border-gray-100 mb-6 flex flex-col md:flex-row justify-between items-center gap-4">
    <form action="{{ route('incidents.index') }}" method="GET" class="flex flex-wrap items-center gap-3 w-full md:w-auto">
        ...
        <div class="relative w-full md:w-64">
            ...
            <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari insiden/pelapor..." 
                   class="pl-9 pr-4 py-2 border border-gray-300 rounded-lg text-sm focus:ring-[#1B4D3E] focus:border-[#1B4D3E] w-full" 
...
<!-- ================= CONFIG WARNA & STATUS (CLEAN CODE) ================= -->
@php
    $severityStyles = [
    ...
    $statusConfig = [
        'insiden_baru'        => ['label' => 'INSIDEN BARU',        'class' => 'bg-purple-100 text-purple-700'],
        'butuh_tindak_lanjut' => ['label' => 'BUTUH TINDAK LANJUT', 'class' => 'bg-orange-100 text-orange-700'],
        'menunggu_verifikasi' => ['label' => 'MENUNGGU VERIFIKASI', 'class' => 'bg-blue-100 text-blue-700'],
            ...
            <tbody class="text-sm">
                ...
                <tr class="border-b border-gray-50 hover:bg-gray-50 transition">
                    ...
                    <td class="py-4 px-6 text-center">
                        ...
                        @else
                            <span class="{{ $severityStyles[$log->severity_level] }} px-2 py-1 rounded text-xs font-bold inline-block uppercase">
                                {{ $log->severity_level }}
                            </span>
                        @endif
                    </td>
                    ...
                    <!-- Rendering Status -->
                    <td class="py-4 px-6 text-center">
                        @if(isset($statusConfig[$log->status]))
                            <span class="{{ $statusConfig[$log->status]['class'] }} px-3 py-1 rounded-full text-xs font-bold whitespace-nowrap">
                        ...
                        <div class="flex items-center justify-center space-x-2">
                            ...
                            @if(session('role') == 'supervisor')
                                ...
                                @if($log->status == 'insiden_baru')
                                    <button onclick="openSeverityModal({{ $log->id }})" title="Set Severity" class="w-7 h-7 flex items-center justify-center rounded-full bg-amber-100 hover:bg-amber-200 text-amber-600 font-black text-sm transition border border-amber-300">!</button>
                                @endif

                                <!-- Tombol Verifikasi -->
                                @if($log->status == 'menunggu_verifikasi')
                                    <button onclick="openVerifyModal({{ $log->id }}, '{{ asset('storage/' . $log->resolution_photo) }}', '{{ addslashes($log->resolution_notes) }}')" title="Verifikasi Laporan" class="w-7 h-7 flex items-center justify-center rounded-full bg-blue-100 hover:bg-blue-200 text-blue-600 transition border border-blue-300">
                            ...
                            @if(session('role') == 'operator')
                                @if(in_array($log->status, ['butuh_tindak_lanjut', 'ditolak']))
