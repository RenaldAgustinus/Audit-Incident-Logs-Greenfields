        ...
        <tbody class="text-sm">
            ...
            <tr class="border-b border-gray-50 hover:bg-gray-50 transition">
                ...
                <td class="py-4 px-6 text-center">
                    <button 
                        onclick="openAuditModal({{ $incident->id }}, '{{ addslashes($incident->incident_title) }}', {{ json_encode($trails[$incident->id] ?? []) }})"
...
<!-- MODAL AUDIT -->
<div id="auditModal" class="hidden fixed inset-0 bg-gray-900 bg-opacity-60 z-50 flex justify-center items-center backdrop-blur-sm">
    <div class="bg-white rounded-xl shadow-2xl w-full max-w-3xl mx-4 overflow-hidden flex flex-col max-h-[85vh]">
        <div class="bg-gray-800 px-6 py-4 flex justify-between items-center text-white">
            <div>
                <h2 class="font-bold text-lg">Riwayat Aktivitas Insiden</h2>
                <p id="modalIncidentTitle" class="text-gray-300 text-sm"></p>
            </div>
            <button onclick="closeModal()" class="text-gray-400 hover:text-white">&times;</button>
        </div>
        <div class="p-6 overflow-y-auto flex-1 bg-gray-50" id="timelineContainer"></div>
        <div class="p-4 border-t bg-white text-right">
            <button onclick="closeModal()" class="px-5 py-2 text-sm font-bold text-gray-600 bg-gray-100 hover:bg-gray-200 rounded-lg">Tutup</button>
        </div>
    </div>
</div>
...
<script>
    const getBadgeClass = (action) => {
        const colors = {
            ...
            'UPDATE_SEVERITY': 'bg-orange-100 text-orange-700',
            'RESOLVE': 'bg-yellow-100 text-yellow-700',
            'VERIFY': 'bg-green-100 text-green-700',
            'APPROVE': 'bg-green-100 text-green-700'
        };
        return colors[action] || 'bg-blue-100 text-blue-700';
    };

    const closeModal = () => document.getElementById('auditModal').classList.add('hidden');
    ...
    function openAuditModal(id, title, trails) {
        ...
        container.innerHTML = trails.length > 0 
            ? `<div class="relative border-l-2 border-green-500 ml-3">${trails.map(renderTrail).join('')}</div>`
            : '<p class="text-gray-500 text-sm italic text-center">Belum ada aktivitas terekam.</p>';

        document.getElementById('auditModal').classList.remove('hidden');
    }

    function renderTrail(trail) {
        const time = new Date(trail.created_at).toLocaleString('id-ID', { day: 'numeric', month: 'short', year: 'numeric', hour: '2-digit', minute: '2-digit' });
        return `
            <div class="mb-8 ml-8 relative">
                ...
                <div class="bg-white p-4 rounded-lg shadow-sm border border-gray-100">
                    <span class="text-xs font-bold ${getBadgeClass(trail.action)} px-2 py-0.5 rounded uppercase">${trail.action}</span>
                    <span class="text-xs text-gray-400 ml-2">${time}</span>
                    <p class="text-gray-800 text-sm font-medium my-2">${trail.description}</p>
